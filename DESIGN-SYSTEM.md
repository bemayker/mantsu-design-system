...
- **DatePicker** typed `DD/MM/YYYY` in every language + optional Monday-first calendar popover. `value`/`onChange` ISO `YYYY-MM-DD` or `null` (never a `Date`), `min`/`max` (inclusive ISO), `label`, `required`, `disabled`, `error`, `hint`, `placeholder`, `locale` (month/weekday names only), `labels` (English defaults, per-key override like Tree), `withCalendar` (default `true`), `size: md|lg`. Separators `/`, `-`, `.`, and also digits with no separator at all (`DDMMYYYY` or `DDMMYY`), for the iPad numeric keypad this field's `inputMode="numeric"` opens, which has no separator key (CORE-FB-23). Four-digit years, separated or not, commit on the keystroke; two-digit years (20xx), separated or not, on blur/Enter; invalid entries keep their text, set `aria-invalid`, never call `onChange`. **`testId` goes on the text input** (not a wrapper, unlike Dropdown); popover ids derive from it: `-toggle`, `-calendar`, `-prev`, `-next`, `-month-label`, `-grid`, `-day-YYYY-MM-DD`, `-today`, `-clear`, `-error`. Never use `<input type="date">` in a Mantsu app (CORE-FB-23).
- **TimeField** 24h `HH:MM` text field, `value`/`onChange` `HH:MM` or `null`; midnight is `00:00`. Accepts `H:MM`/`HH:MM` (`:` or `.`) and, for the same numeric-keypad reason, digits with no separator (`HHMM` or `HMM`). `testId` on the input. Replaces `<input type="time">`.
- **DateTimePicker** DatePicker + TimeField side by side, `value`/`onChange` `YYYY-MM-DDTHH:MM` or `null` (the `datetime-local` shape); reports only when both halves are valid. `datePlaceholder`/`timePlaceholder` pass through to each half (defaults unchanged: `DD/MM/YYYY`, `HH:MM`). `onValidityChange?: (valid: boolean) => void` (1.6.0) tells a consumer whenever what the fields currently show, judged from the current text of each half rather than its last committed value, flips between reportable and not: `false` for an invalid or not-yet-committed-but-unresolvable half, or for exactly one half holding text while the other is empty; `true` when both are empty or both resolve (a pending two-digit year counts as valid). Fires once on mount, then only on change, so a form can hold Save without reading the DOM. `testId` on the group; inputs `${testId}-date`, `${testId}-time`. Replaces `<input type="datetime-local">`.
